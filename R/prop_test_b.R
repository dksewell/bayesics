...
prop_test_b = function(n_successes,
    ...
    list(name = 
           paste0("Analysis of ",
                  c("a single population proportion",
                    "two population proportions")[length(n_successes)]),
         data = tibble(`Number of successes` = n_successes,
                       `Number of failures`  = n_failures),
         print_data = TRUE,
         CI_level = CI_level,
         prior = 
           paste0("Prior on the population proportion: Beta(",
                  prior_shapes[1],
                  ", ",
                  prior_shapes[2],
                  ")"))
  ...
  if(length(n_successes) == 1){
    ...
    # Compute results
    results$results = 
      tibble(Quantity = "Population proportion",
             `Post Mean` = 
    ...
    if(!missing(p)){
      ...
      results$pdir$description = 
        paste0("Probability that the population proportion is ",
    ...
    results$results = 
      tibble(Quantity = 
               c("Population 1 proportion",
                 "Population 2 proportion",
                 "Difference in proportions (Pop 1 - Pop 2)"),
             `Post Mean` = 
               c(post_shapes[1,1] / sum(post_shapes[1,]),
                 post_shapes[2,1] / sum(post_shapes[2,]),
                 mean(p1_draws - p2_draws)),
             Lower = 
               c(qbeta(0.5 * alpha_ci,
                       post_shapes[1,1],
                       post_shapes[1,2]),
                 qbeta(0.5 * alpha_ci,
                       post_shapes[2,1],
    ...
    results$overall_ROPE = 
      list(ROPE_lower_bound = ROPE[1],
           ROPE_upper_bound = ROPE[1],
           description = 
             paste0("Probability that the odds ratio (Pop 1 vs. Pop 2) is between ",
    ...
    results$pdir$description = 
      paste0("Probability that the odds ratio (Pop 1 vs. Pop 2) is ",
             ifelse(results$pdir$pdir > 0.5,
                    "less",
                    "greater"),
             " than 1")
