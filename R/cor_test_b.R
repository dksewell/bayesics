...
cor_test_b.default = function(x,
  ...
  # Collate results
  results = 
    list(name = "Analysis of Kendall's tau",
         data = tibble(x=x,
                       y=y),
         print_data = FALSE,
         CI_level = CI_level,
         prior = 
           paste0("Prior on the proportion for concordance is: Beta(",
                  prior_shapes[1],
                  ", ",
                  prior_shapes[2],
                  ")"))
  ...
  results$results = 
    tibble(Quantity = "Kendall's tau",
           `Post Mean` = 
             ...
             1.0,
           Lower = 
             2.0 * dfba_object$eti_lower - 1.0,
           Upper = 
             2.0 * dfba_object$eti_upper - 1.0,
           Pr_in_ROPE = 
  ...
  results$pdir$description = 
    paste0("Probability that tau is ",
